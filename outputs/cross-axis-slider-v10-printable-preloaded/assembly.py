"""Explicit V10 assembly placements for static travel poses."""
...
def _shoulder_fastener(
    ...
    thread_far_x = bearing_far_x + direction * 3.4
...
def roller_stack_report() -> Dict[str, object]:
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        keyboard_rail_x = sign * SIDE_FRAME_X + (16.6 if side == "right" else -16.0)
        trackpad_rail_x = sign * SIDE_FRAME_X + (17.1 if side == "right" else -16.5)
...
class PoseAssembly:
    ...
    def interface_report(self) -> Dict[str, object]:
        ...
        for side in ("left", "right"):
            ...
            if side == "left":
                pairs.extend([
                    ("keyboard_tray", "keyboard_carriage_left"),
                    ("trackpad_tray", "trackpad_carriage_left"),
                ])
            else:
                for kind in ("keyboard", "trackpad"):
                    washer = f"{kind}_float_washer_right_1"
                    pairs.extend([
                        (f"{kind}_tray", washer),
                        (washer, f"{kind}_carriage_right"),
                    ])
    ...
    def preload_report(self) -> Dict[str, float]:
        ...
        return {
            "maximum_adjusted_wall_gap_mm": max(abs(guide_gap), abs(cross_gap)),
...
def build_pose(travel: float, include_devices: bool = False) -> PoseAssembly:
    ...
    for foot_index, (foot_x, foot_y) in enumerate(
        ((-137.0, -101.0), (-137.0, 131.0), (137.0, -101.0), (137.0, 131.0)),
        1,
    ):
        add(
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        for kind, translation, carriage_x in (
            ...
            if kind == "keyboard":
                local_rail_x = 16.6 if side == "right" else -16.0
            else:
                local_rail_x = 17.1 if side == "right" else -16.5
        ...
        # Guided compression spring; fixed endpoint is invariant by construction.
        ...
        add(
            f"compression_spring_{side}",
            _spring_world(keyboard_carriage_x, t, moving_seat),
            SPRING_COLOR,
        )
        add(
        ...
        for stop_name, stop_center in (
            ...
            add(
                ...
                _axis_oriented_part(
                    parts.make_tpu_stop_collar(), keyboard_carriage_x, stop_center
                ),
                TPU_COLOR,
            )
        ...
        for kind, tray_position, ear_y, hole_z, carriage_x, screw_length in (
            ...
            for mount_index, y_offset in enumerate((-10.0, 10.0), 1):
                ...
                if side == "right":
                    washer_x = 145.5 + p.RIGHT_SIDE_AXIAL_FLOAT / 2.0
                    add(
                        f"{kind}_float_washer_right_{mount_index}",
                        parts.make_right_float_washer().moved(
                            Pos(washer_x, mount_y, mount_z)
                        ),
                        TPU_COLOR,
                    )

    compound = Compound(
        label=f"cross_axis_slider_v10_travel_{t:.3f}",
        children=children,
    )
    return PoseAssembly(
        compound=compound,
