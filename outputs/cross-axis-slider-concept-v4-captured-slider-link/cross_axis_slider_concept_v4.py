"""V4 captured, single-side roller-slot linkage for the cross-axis concept."""
...
def _slot_geometry(key_y, key_z, track_y, track_z):
    rel_dy = v1.KEYBOARD_DY - v1.TRACKPAD_DY
    rel_dz = v1.KEYBOARD_DZ - v1.TRACKPAD_DZ
    pin_y = key_y + 30.0
    pin_z = key_z + 21.0

    slot_center_y0 = (v1.KEYBOARD_Y0 + 30.0) + rel_dy / 2.0
    slot_center_z0 = (v1.KEYBOARD_Z0 + 21.0) + rel_dz / 2.0
    ...
    return pin_y, pin_z, slot_center_y, slot_center_z, slot_angle


def _keyboard_driver(key_y, key_z, pin_y, pin_z):
    """Two-point triangular bracket rigidly overlapping the keyboard carriage."""
...
def _trackpad_cheek(cheek_x, cheek_name, track_y, track_z,
    ...
    # Two independent necks provide a real two-point rigid attachment.
    anchor_front = (track_y - 15.0, track_z - 2.0)
    anchor_rear = (track_y + 15.0, track_z - 2.0)
    attach_front = (slot_center_y + 4.0, slot_center_z + 4.0 * (3.0 / 145.0))
    attach_rear = (slot_center_y + 38.0, slot_center_z + 38.0 * (3.0 / 145.0))
...
def _captured_linkage(key_y, key_z, track_y, track_z):
    ...
    children = [
        ...
        _trackpad_cheek(
            OUTER_CHEEK_X,
            "trackpad_slotted_cheek_outer_left",
            track_y,
            track_z,
            slot_y,
            slot_z,
        ...
        _trackpad_cheek(
            INNER_CHEEK_X,
            "trackpad_slotted_cheek_inner_left",
            track_y,
            track_z,
            slot_y,
            slot_z,
    ...
    # Two transverse ties visibly clamp both cheeks to the trackpad carriage.
...
def kinematic_sample(travel):
    ...
    track_z = v1.TRACKPAD_Z0 + t * v1.TRACKPAD_DZ
    ...
    rel_dy = v1.KEYBOARD_DY - v1.TRACKPAD_DY
    rel_dz = v1.KEYBOARD_DZ - v1.TRACKPAD_DZ
    rel_len = hypot(rel_dy, rel_dz)
    along = ((pin_y - slot_y) * rel_dy + (pin_z - slot_z) * rel_dz) / rel_len
    cross = ((pin_y - slot_y) * rel_dz - (pin_z - slot_z) * rel_dy) / rel_len
...
def validate_kinematics(sample_count=201):
    ...
    min_margin = min(sample["roller_center_margin"] for sample in samples)
    monotonic = all(
    ...
    return {
        ...
        "minimum_roller_center_margin": min_margin,
        "monotonic": monotonic,
    }
...
def build_pose(travel=0.0, pose_name="keyboard_mode"):
    ...
    return Compound(
        label=f"cross_axis_slider_concept_v4_captured_link_{pose_name}",
        children=children,
    )
