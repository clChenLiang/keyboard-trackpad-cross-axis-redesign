"""Deterministic numerical and BREP validation for V11."""
...
def _designed_contact(label_a: str, label_b: str) -> bool:
    ...
    for side in ("left", "right"):
        ...
        if pair == frozenset((f"compression_spring_{side}", f"spring_guide_rod_{side}")):
            return True
...
def validate_collision_pose(travel: float, volume_tolerance: float = 0.05) -> Dict:
    ...
    bbox_candidates = 0
    pose = assembly.build_pose(travel, include_visual_springs=False)
...
def validate_collisions(sample_count: int = 101, volume_tolerance: float = 0.05) -> Dict:
    ...
    # Three isolated kernels balance wall time and memory.  Four detailed CAD
    # workers were previously terminated by the OS on this machine.
    with ThreadPoolExecutor(max_workers=min(3, sample_count)) as executor:
...
def validate_spring() -> Dict:
    ...
    strongest_10_percent_maximum_user = (
        end_force * 1.10 - gravity_equivalent + ROLLING_RESISTANCE_N
    )
    compressed_length = p.SPRING_INSTALLED_LENGTH - mech.KEYBOARD_PATH
    coil_bind_margin = compressed_length - p.SPRING_SOLID_LENGTH
    mean_diameter = p.SPRING_OD - p.SPRING_WIRE
    spring_index = mean_diameter / p.SPRING_WIRE
    wahl_factor = (
        (4.0 * spring_index - 1.0) / (4.0 * spring_index - 4.0)
        + 0.615 / spring_index
    )
    end_force_per_spring = (
        p.SPRING_FREE_LENGTH - compressed_length
    ) * p.SPRING_RATE_PER_SPRING
    maximum_shear_stress = (
        8.0 * end_force_per_spring * mean_diameter * wahl_factor
        / (pi * p.SPRING_WIRE ** 3)
    )
    shear_safety_factor = p.SPRING_SHEAR_ALLOWABLE_MPA / maximum_shear_stress
    sleeve_diametral_clearance = (
        p.SPRING_OD - 2.0 * p.SPRING_WIRE - p.SPRING_GUIDE_SLEEVE_OD
    )
    calculated_rate = (
        p.SPRING_SHEAR_MODULUS_MPA * p.SPRING_WIRE ** 4
        / (8.0 * mean_diameter ** 3 * p.SPRING_ACTIVE_COILS)
    )
    rate_error_fraction = abs(calculated_rate - p.SPRING_RATE_PER_SPRING) / p.SPRING_RATE_PER_SPRING
    return {
        "keyboard_moving_mass_kg": keyboard_mass,
        "trackpad_moving_mass_kg": trackpad_mass,
        ...
        "end_force_per_spring_n": end_force_per_spring,
        "spring_index": spring_index,
        "wahl_factor": wahl_factor,
        "maximum_shear_stress_mpa": maximum_shear_stress,
        "shear_allowable_mpa": p.SPRING_SHEAR_ALLOWABLE_MPA,
        "spring_shear_safety_factor": shear_safety_factor,
        "guide_sleeve_diametral_clearance_mm": sleeve_diametral_clearance,
        "guide_rod_length_mm": p.SPRING_INSTALLED_LENGTH + 42.0,
        "calculated_rate_n_per_mm": calculated_rate,
...
def validate_all(motion_samples: int = 401, collision_samples: int = 101) -> Dict:
    ...
    if spring["strongest_10_percent_maximum_user_force_n"] > 23.0:
    ...
    if spring["spring_shear_safety_factor"] < 1.25:
        errors.append({"spring_shear": spring})
    if not 4.0 <= spring["spring_index"] <= 15.0:
        errors.append({"spring_index": spring})
    if spring["guide_sleeve_diametral_clearance_mm"] < 0.4:
        errors.append({"spring_guide_sleeve_clearance": spring})
    if spring["rate_error_fraction"] > 0.05:
        errors.append({"spring_rate_consistency": spring})
