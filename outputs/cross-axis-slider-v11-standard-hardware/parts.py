...
def make_side_frame(side: str):
    ...
    keyboard_x = 16.1 if side == "right" else 15.5
    trackpad_x = 20.1 if side == "right" else 19.5
...
def make_carriage(kind: str, side: str):
    ...
    if kind == "trackpad":
        ...
        # V11 moves the trackpad carriage layer 1.5 mm outward to accept a
    ...
    if kind == "keyboard":
        ...
        spring_seat = _capsule_yz(
            20.0, 12.0, 12.5, moving_seat[0], moving_seat[1],
            degrees(atan2(mech.KEYBOARD_U[1], mech.KEYBOARD_U[0])) + 90.0,
            spring_x,
        )
    ...
    if kind == "trackpad":
        ...
        tray_sweep_relief = Box(
            ...
            21.0,
            align=(Align.CENTER, Align.CENTER, Align.CENTER),
        ).moved(Pos(inner_sign * 8.25, 52.0, 57.5))
