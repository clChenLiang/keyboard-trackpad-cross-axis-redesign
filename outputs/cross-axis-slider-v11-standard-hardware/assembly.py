"""Explicit V11 assembly placements for static travel poses."""
...
def roller_stack_report() -> Dict[str, object]:
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        keyboard_rail_x = sign * SIDE_FRAME_X + (16.1 if side == "right" else -15.5)
        trackpad_rail_x = sign * SIDE_FRAME_X + (20.1 if side == "right" else -19.5)
...
def build_pose(
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        for kind, translation, carriage_x in (
            ...
            if kind == "keyboard":
                local_rail_x = 16.1 if side == "right" else -15.5
            else:
                local_rail_x = 20.1 if side == "right" else -19.5
        ...
        spring_shape = (
            _spring_world(spring_x, t, moving_seat)
            if include_visual_springs
            else spring_collision_envelope(spring_x, t, moving_seat)
        )
        add(f"compression_spring_{side}", spring_shape, SPRING_COLOR)
        add(
    ...
    compound = Compound(
        label=f"cross_axis_slider_v11_travel_{t:.3f}",
        children=children,
    )
    return PoseAssembly(
        compound=compound,
