...
class GeometryTests(unittest.TestCase):
    ...
    def test_heat_set_pilots_are_not_loose_clearance_holes(self):
        self.assertLess(p.M4_HEAT_INSERT_HOLE_DIAMETER, p.M4_HEAT_INSERT_OD)
        self.assertGreaterEqual(
            p.M4_HEAT_INSERT_OD - p.M4_HEAT_INSERT_HOLE_DIAMETER, 0.4
        )
        self.assertLess(
            p.RETURN_CLAMP_INSERT_HOLE_DIAMETER, p.RETURN_CLAMP_INSERT_OD
        )
        self.assertGreaterEqual(
            p.RETURN_CLAMP_INSERT_OD - p.RETURN_CLAMP_INSERT_HOLE_DIAMETER, 0.4
        )
    ...
    def test_same_stack_hardware_has_no_solid_interference(self):
        for travel in (0.0, 0.5, 1.0):
            report = build_pose(travel).hardware_interference_report()
            self.assertEqual([], report["failures"], report)

    def test_left_datum_preloads_and_right_side_remains_a_follower(self):
        for travel in (0.0, 0.5, 1.0):
            report = build_pose(travel).preload_report()
            self.assertLessEqual(report["left_datum_maximum_adjusted_wall_gap_mm"], 0.05)
            ...
            self.assertAlmostEqual(0.50, report["right_side_normal_float_mm"], places=6)
            self.assertAlmostEqual(0.50, report["right_fixed_roller_nominal_clearance_mm"], places=6)
            self.assertAlmostEqual(0.10, report["right_eccentric_minimum_clearance_mm"], places=6)
            self.assertFalse(report["right_side_nominally_preloaded"])
    ...
    def test_shoulder_lengths_are_exact_and_bilateral(self):
        ...
        self.assertEqual(1.3, report["head_thickness_mm"])
    ...
    def test_spring_force_stress_and_guide_are_consistent(self):
        ...
        self.assertLessEqual(report["maximum_user_force_n"], 8.0, report)
        self.assertLessEqual(report["strongest_10_percent_maximum_user_force_n"], 10.0, report)
        self.assertGreaterEqual(report["working_deflection_margin_mm"], 0.0, report)
        self.assertGreaterEqual(report["rated_life_cycles"], 10000, report)
        self.assertEqual("MISUMI CFS0.2", report["part_number"])
    ...
    def test_roller_detail_has_selectable_complete_stacks(self):
        for exploded in (False, True):
            ...
            for stack in ("guide_fixed", "guide_eccentric", "cross_eccentric", "right_follower"):
                for suffix in ("bearing", "shaft", "retaining_washer", "retaining_nut"):
