...
def base_plate():
    return Compound(label="v12_base_plate", children=[
        _place("base", parts.make_open_base(), 0.0, 0.0),
    ])


def trays_plate():
    return Compound(label="v12_trays_plate", children=[
...
def frames_plate():
    return Compound(label="v12_frames_plate", children=[
...
def carriages_plate():
    return Compound(label="v12_carriages_plate", children=[
        ...
        _place("keyboard_carriage_left", parts.make_carriage("keyboard", "left"), 48.0, -61.0),
        _place("keyboard_carriage_right", parts.make_carriage("keyboard", "right"), 48.0, 61.0),
    ])
...
def petg_small_parts_plate():
    ...
    return Compound(label="v12_petg_small_parts_plate", children=children)
...
def tpu_parts_plate():
    ...
    for index in range(2):
        children.append(_place(
            f"tpu_spool_retainer_{index + 1}",
            parts.make_tpu_spool_retainer(),
            70.0 + index * 22.0,
            -50.0,
        ))
    return Compound(label="v12_tpu_parts_plate", children=children)


def small_parts_plate():
    """Legacy combined review plate; manufacturing uses two material plates."""
    return Compound(
        label="v12_small_parts_plate_review_only",
